...
public class AuthenticationControler {
    ...
    ApiRespon<AuthenticationResponse> authenticate(@RequestBody AuthenticationRequest request) throws JOSEException {
        ...
        //                        .authenticated(true)
        //                    .build());
        return ApiRespon.<AuthenticationResponse>builder().result(result).build();
    }

    @PostMapping("/introspect")
    ApiRespon<IntrospectResponse> introspect(@RequestBody IntrospectRequest request)
            throws JOSEException, ParseException {
        var result = authenticationService.introspectResponse(request);
        return ApiRespon.<IntrospectResponse>builder().result(result).build();
    }

    @PostMapping("/refresh")
    ApiRespon<AuthenticationResponse> introspect(@RequestBody RefreshTokenRequest request)
            throws JOSEException, ParseException {
