...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity.authorizeHttpRequests(request -> request.requestMatchers(HttpMethod.POST, PUBLIC_ENOPOINTS)
                .permitAll() /// chỉ ddịnh các đường dẫn post, get...
                //                        .requestMatchers(HttpMethod.GET, "/users").hasRole(Role.ADMIN.name())
                // .hasAnyAuthority("ROLE_ADMIN") cach tu cau hinh
                .anyRequest()
                .authenticated()); // còn lại phải được xác thực

        httpSecurity.oauth2ResourceServer(
                oauth2 -> // bảo vệ tài nguyên bằng cách xác thực token
                oauth2.jwt(jwtConfigurer -> jwtConfigurer
                                .decoder(customJwtDecoded)
                                .jwtAuthenticationConverter(jwtAuthenticationConverter()))
                        .authenticationEntryPoint(
                                new JwtAuthenticationEntryPoint())); // Khi request không nhận được token hoợp lệ hoawjc
        // không có
